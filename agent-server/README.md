...
# ── Database ───────────────────────────────────────────────
# agent_user, your_password, localhost, 5432, agent_db를 실제 값으로 교체
DATABASE_URL=postgresql+asyncpg://agent_user:your_password@localhost:5432/agent_db

# ── 허용 파일 경로 ──────────────────────────────────────────
# AI가 접근할 수 있는 디렉토리 목록 (쉼표 없이 JSON 배열 형식)
ALLOWED_DIRECTORIES=["C:/Users/KIHOPARK/Projects","C:/Users/KIHOPARK/Documents"]

# ── 웹 검색 (선택) ─────────────────────────────────────────
SEARCH_PROVIDER=brave          # brave | duckduckgo
BRAVE_API_KEY=BSA-...          # Brave Search API Key (brave 사용 시 필수)
```
...
| `CLAUDE_MODEL` | `claude-3-5-sonnet-20241022` | 사용할 Claude 모델 |
...
| `SEARCH_PROVIDER` | `brave` | 검색 공급자 (`brave` \| `duckduckgo`) |
